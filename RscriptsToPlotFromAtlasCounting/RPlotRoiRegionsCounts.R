...
library("gridExtra")
##### Auxiliary functions#########
plotCountsC <- function(data,index){
  
  plt <- ggplot(data,aes(x = as.numeric(`Number Slice`)))+
    geom_point( aes(y = as.numeric(`Number of Labels`), color = "Total"), size = 1.5) +
    geom_line( aes(y = as.numeric(`Number of Labels`), color = "Total"), linetype = "solid", linewidth = 0.5) +
    
    geom_point(aes( y = as.numeric(`Number of Labels Left`),color ="Left"), size = 1.5) +
    geom_line( aes( y = as.numeric(`Number of Labels Left`), color = "Left"), linetype = "dashed", linewidth = 0.5) +
    
    geom_point( aes( y = as.numeric(`Number of Labels Right`),color = "Right"), size = 1.5) +
    geom_line( aes( y = as.numeric(`Number of Labels Right`), color = "Right"), linetype = "dotted", linewidth = 0.5)+
    labs(title = data[1,]$`Region considered`)+xlab("slice number") + ylab("counts") +
...
for(s in sorted_sheet_names){
  ...
  plt[[index]] <- plotCountsC(data,index)
  index = index + 1
  print(s)
 }
