...
def OK_action():
    ...
    elif check_var2.get() and check_var1.get():
     #find the labels
     ManagerData(Folder_with_data, Type_of_stain,scale)
     #count the labels
     ManagerCounting(Folder_with_data, Type_of_stain,scale)
...
def main():
   ...
   ok_button.grid(row=8, column=0, columnspan=2, pady=30)
   ...
   checkbutton1 = tk.Checkbutton(app, text="Automatic counting", variable=check_var1, font=('Helvetica', 12,'bold'), bg='lightblue', fg='black')
   ...
   label_text_box.grid(row=5, column=1, pady=5)
   # Pack the checkbuttons
   checkbutton1.grid(row=6, column=1, sticky='w',pady =20)
   checkbutton2.grid(row=7, column=1, sticky='w',pady =20)
